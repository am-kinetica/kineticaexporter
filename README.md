...
- Please refer to https://github.com/open-telemetry/opentelemetry-collector/tree/main/cmd/builder for instructions on how to get the latest binary and install locally.
...
dist:
  ...
  output_path: /home/anindyam/kinetica/otelexporter_utils/collector-binary
  otelcol_version: 0.78.2

exporters:
  - gomod:
      github.com/am-kinetica/kineticaexporter v0.0.36
contrib/exporter/fileexporter v0.78.0
...
- Once the builder config file has been created the binary can be build using the command `ocb/builder --config config_file.yaml`. The name of the collector builder binary could either of `ocb|builder` depending on how the builder binary has been installed.  
...
2. The Processor - the metric data received from the `Receiver`s will be passed on to this component so that the if needed 
...
exporters:
  kinetica:
    host: https://demo.kinetica.com/_gpudb
    schema: otel
    username: admin
    password: password
...
The first section of the config file describes the components (receivers, exporters etc.) with their configurations. The second section describes the `service:` details which has the details for the pipeline stages described for logs, traces and metrics.  Let's say this file has been saved as `collector-config.yaml`.

The command to run the collector binary produced by the steps described above is `<name_of_the_binary> --config <name_of_the_config_file`
